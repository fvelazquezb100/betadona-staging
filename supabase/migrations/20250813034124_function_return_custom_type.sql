-- Create a custom type for the function return value
CREATE TYPE public.username_update_result AS (
  success boolean,
  message text
);

-- Create the update_username RPC function
...
BEGIN
  ...
  IF length(trim(new_username)) > 50 THEN
    result.success := false;
    result.message := 'Username must be 50 characters or less';
    RETURN result;
  END IF;
